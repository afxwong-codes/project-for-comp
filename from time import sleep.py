...
print("""
                                ...
                                `--'   '--'`------`-----'   `-----'  `-----'`--'   `--'`------'                          
                                                        ...
                                                        `--.  .--( _) | |  |                                             
                                                           ...
                                                           `--'     `-----'                                              
<-. (`-')              (`-')_(`-')   (`-')  _  (`-')     <-. (`-')            (`-').-(`-')     (`-')  _  (`-')           
   \(OO )_     .->  <-.(OO ( (OO ).->( OO).-<-.(OO )        \(OO )_     .->   ( OO)_ ( OO).->  ( OO).-<-.(OO )     .->   
,--./  ,-.,--.(,--. ,------,\    .'_(,------,------,)    ,--./  ,-.),--.'  ,-(_)--\_)/    '._ (,------,------,),--.'  ,-.
|   `.'   |  | |(`-'|   /`. '`'-..__)|  .---|   /`. '    |   `.'   (`-')'.'  /    _ /|'--...__)|  .---|   /`. (`-')'.'  /
|  |'.'|  |  | |(OO |  |_.' |  |  ' (|  '--.|  |_.' |    |  |'.'|  (OO \    /\_..`--.`--.  .--(|  '--.|  |_.' (OO \    / 
|  |   |  |  | | |  |  .   .|  |  / :|  .--'|  .   .'    |  |   |  ||  /   /).-._)   \  |  |   |  .--'|  .   .'|  /   /) 
...
def get_attempted_input(prompt, valid_answers):
    ...
    while attempts < 3:
        ...
        for valid in valid_answers:
            valid_norm = ' '.join(valid.strip().lower().split())
            if answer == valid_norm:
                return valid  
        attempts += 1
        if attempts < 3:
...
choice = get_attempted_input(
    "find special objects to increase your score, where do you look first?",
    first_choices
)
while True:
    if choice in ["investigate living room", "investigate the living room"]:
        ...
        if investigatescene:
            print("the body belonged to the person who lived in room 4")
            sleep(2)
            ...
            sleep(2)
            wherenext = get_attempted_input("where do you want to investigate next?", ["investigate room 4", "investigate 4", "investigate clair's room"])
            if wherenext in ["investigate kitchen"]:
            ...
            elif wherenext in ["investigate garden"]:
                ...
                shed_choice = get_attempted_input("do you want to try to open the shed? enter yes, no, or you can knock", ["yes", "no", "knock"])
                if shed_choice == "yes":
                    print("you shove the door with all your strength an it hits you against the fall as it opens")
                    sleep(2)
                    print("""you fall over and pass out as you finally understand that you should have listened to your parents
                           and learned to knock before breaking into a room, or shed or whatever""")
                ...
                if pick in ["look in closet", "investigate the closet", "investigate closet"]:
                    ...
                    unlockmystery = get_attempted_input("what would you like to do now?", ["pick up key", "investigate key", "use key", "unlock trapdoor"])
                    if unlockmystery:
                        print("you slot the key into the lock in the trapdoor and it makes a satisfying click")
                        sleep(2)
                        print("inside is a dusty box, which is empty, apart from an old note")
                        sleep(1)
                        print("""it reads "to clair, this is what I give you now I have passed, 
                                            keep it safe for when you need it most
                                            - your mother" """)
                        sleep(3)
                        print("you realise the murderer has goten here before you and stolen something, money?")
                        sleep(1)
                        print("ending: A little too late, score", score)
                        sys.exit()
        ...
        if garden_action == "footprints":
            ...
            print("You notice a torn piece of red fabric caught in a bush.")
        ...
        for part in reversed(parts):
            if part.isdigit():
                ...
                break
        if not room_num:
            print("Could not determine which room to investigate.")
            sys.exit()
        print(f"You enter room {room_num}. It's quiet. You notice a diary on the desk.")
        sleep(1)
        if room_num == "9":
